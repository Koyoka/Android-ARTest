...
public class FragmentMediaHelper {
...
//	private static final int MEDIA_IMG_CROP_FINISH = 32;
	...
	private static Fragment mCurActy1;
	...
	public static void setMedia(Fragment acty,int mediaType,String path) {
//		mCurActy = acty;
//		curMediaType = mediaType;
	...
	private static void getMedia_Photo(Fragment acty){
	...
	private static void getMedia_Camera(Fragment acty,String path){
...
//			MessageBox.getInstance(mCurActy).Show("请插入SD卡");
			ToastUtil.show(acty.getActivity(), "请插入SD卡");
		}
...
//			curMediaType = MEDIA_IMG_CAMERA;
	...
	public static String getMedioResultPath(Fragment mCurActy,Intent data,int resultCode,String savePath){
...
//		String savePath = data.getStringExtra(INTENT_KEY_MEDIA_PHOTOFILEPATH);
		...
		case FragmentMediaHelper.MEDIA_IMG_CAMERA:
...
//			DebugTrace.Print("4,---------"+uri);
			if(uri != null){
				if((curMediaType&BIT_HIGH) == MEDIA_IMG_CROP){
					...
					imgPath = uri.getPath();
					galleryAddPic(mCurActy.getActivity(), imgPath);
				}
			}
//			mCurrentPhotoFile = null;
			break;
		case FragmentMediaHelper.MEDIA_IMG_PHOTO:
			if(data != null){
...
//    			Cursor c = mCurActy.managedQuery(uri, null, null, null, null);
    			...
    			Cursor c = mCurActy.getActivity().getContentResolver().query(uri,
...
//			MessageBox.getInstance(mCurActy).Show("curMediaType & AND_HIGH");
			if(uri!=null){
				getMedia_CropImage(mCurActy.getActivity(),uri,savePath);
			}
